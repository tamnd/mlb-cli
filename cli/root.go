// Package cli assembles the mlb command tree from the mlb
...
// NewApp assembles the kit application from the mlb domain. The
...
//
// To add a command, declare it in mlb/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := mlb.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(mlb.Domain{}).Register(app)
