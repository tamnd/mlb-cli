...
describes what users get. Pick a format with `-o`, or let mlb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mlb <command> -o table   # aligned columns for reading
mlb <command> -o jsonl   # one JSON object per line, for piping
mlb <command> -o json    # a single JSON array
mlb <command> -o csv     # spreadsheet friendly
mlb <command> -o tsv     # tab-separated
mlb <command> -o url     # just the URL column
mlb <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mlb <command> --fields id,title,url
```
...
```bash
mlb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mlb <command>            # a table, because this is a terminal
mlb <command> | wc -l    # JSONL, because this is a pipe
```
